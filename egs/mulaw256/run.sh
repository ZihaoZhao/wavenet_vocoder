...
exp_dir=/zhzhao/code/wavenet-vocoder/egs/mulaw256/

hparams=${exp_dir}/conf/mulaw256_wavenet.json
...
expdir=${exp_dir}/exp/$expname

feat_typ="logmelspectrogram"

# Output directories
data_root=${exp_dir}/data/$spk                        # train/dev/eval splitted data
dump_org_dir=${exp_dir}/$dumpdir/$spk/$feat_typ/org   # extracted features (pair of <wave, feats>)
dump_norm_dir=${exp_dir}/$dumpdir/$spk/$feat_typ/norm # extracted features (pair of <wave, feats>)
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    /zhzhao/miniconda3/envs/pytorch16/bin/python $VOC_DIR/train.py --dump-root $dump_norm_dir --preset $hparams \
      ...
      --log-event-path=${exp_dir}/tensorboard/${expname}
fi
